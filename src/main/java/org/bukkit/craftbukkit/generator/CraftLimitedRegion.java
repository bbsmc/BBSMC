...
public class CraftLimitedRegion extends CraftRegionAccessor implements LimitedRegion {
    ...
    public net.minecraft.world.level.WorldGenLevel getHandle() {
        ...
        if (handle == null) {
            throw new IllegalStateException("net.minecraft.world.level.WorldGenLevel no longer present, are you using it in a different tick?");
        }
    ...
    public void setBiome(int x, int y, int z, Holder<net.minecraft.world.level.biome.Biome> biomeBase) {
        Preconditions.checkArgument(isInRegion(x, y, z), "Coordinates %s, %s, %s are not in the region", x, y, z);
        net.minecraft.world.level.chunk.ChunkAccess chunk = getHandle().getChunk(x >> 4, z >> 4, ChunkStatus.EMPTY);
