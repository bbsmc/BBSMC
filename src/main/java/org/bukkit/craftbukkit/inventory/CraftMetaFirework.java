...
class CraftMetaFirework extends CraftMetaItem implements FireworkMeta {
    ...
    CraftMetaFirework(net.minecraft.nbt.CompoundTag tag) {
        super(tag);
        ...
        net.minecraft.nbt.CompoundTag fireworks = tag.getCompound(FIREWORKS.NBT);
        ...
        net.minecraft.nbt.ListTag fireworkEffects = fireworks.getList(EXPLOSIONS.NBT, CraftMagicNumbers.NBT.TAG_COMPOUND);
        List<FireworkEffect> effects = this.effects = new ArrayList<FireworkEffect>(fireworkEffects.size());

        for (int i = 0; i < fireworkEffects.size(); i++) {
            try {
                effects.add(getEffect((net.minecraft.nbt.CompoundTag) fireworkEffects.get(i)));
    ...
    static FireworkEffect getEffect(net.minecraft.nbt.CompoundTag explosion) {
    ...
    static net.minecraft.nbt.CompoundTag getExplosion(FireworkEffect effect) {
    ...
    void applyToItem(net.minecraft.nbt.CompoundTag itemTag) {
        ...
        net.minecraft.nbt.CompoundTag fireworks = itemTag.getCompound(FIREWORKS.NBT);
    ...
    static void addColors(net.minecraft.nbt.CompoundTag compound, ItemMetaKey key, List<Color> colors) {
        if (colors.isEmpty()) {
            return;
        }
